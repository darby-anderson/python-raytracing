...
if __name__ == '__main__':
    ...
    mesh = Mesh.from_stl("unit_sphere.stl", np.array([1.0, 0.0, 1.0]),\
        np.array([1.0, 1.0, 1.0]),0.0,1.0,0.2,1.0, 0.2)
    mesh.transform.set_rotation(0, 45, 45)
    mesh.transform.set_position(1, 4, 0)

    mesh2 = Mesh.from_stl("unit_sphere.stl", np.array([0.0, 0.0, 1.0]), \
                          np.array([1.0, 1.0, 1.0]), 0.2, 1.0, 0.0, 1.0, 0.2)
    mesh2.transform.set_position(-1, 4, 0)

    light = PointLight(50.0, np.array([1, 1, 1]))
    light.transform.set_position(-2, -1, 0)
